...
pub fn basic_meta(config: &impl LinkConfig) -> Markup{
    html! {
        ...
        link rel="icon" type="image/svg+xml" sizes="any" href={(config.get_resource_root()) "/favicon.svg"};
    } 
}

//Render basic navigation link with only text as the body
pub fn main_nav_link(config: &impl LinkConfig, text: &str, href: &str, current_path: &str, id: Option<&str>) -> Markup {
...
pub fn main_nav_link_raw(config: &impl LinkConfig, body: Markup, href: &str, current_path: &str, id: Option<&str>) -> Markup {
    let mut class = String::from("plainlink headertab");
    if current_path.starts_with(href) { class.push_str(" current"); }
    html! {
        a.(class) href={(config.get_http_root()) (href)} id=[id] { (body) }
    }
}

//Produce the footer for the main selection of pages
pub fn footer(config: &impl LinkConfig, about_api: &api::About, current_path: &str) -> Markup {
    html! {
...
pub fn header_user_inner(config: &impl LinkConfig, user: &api::User) -> Markup {
    html! {
        span { (user.username) }
...
pub fn header(config: &impl LinkConfig, current_path: &str, user: Option<&api::User>) -> Markup {
    html! {
        header."controlbar" {
            nav {
                a."plainlink" #"homelink" href={(config.get_http_root())"/"}{
                    img src={(config.get_resource_root())"/favicon.ico"};
...
pub fn style(config: &impl LinkConfig, link: &str, cache_bust: &str) -> Markup {
    html! {
        link rel="stylesheet" href={(config.get_static_root()) (link) "?" (cache_bust) } { }
    }
}

pub fn script(config: &impl LinkConfig, link: &str, cache_bust: &str) -> Markup {
    html! {
        script src={(config.get_static_root()) (link) "?" (cache_bust) } { }
    }
}

pub struct MainLayoutData<'a> {
    pub config: Box<dyn LinkConfig>,
    pub user_config: Box<dyn UserConfig>,
...
pub fn layout(main_data: MainLayoutData<'_>, page: Markup) -> Markup {
    html! {
        (DOCTYPE)
        html lang=(main_data.user_config.get_language()) {
            head {
                (basic_meta(main_data.config.as_ref()))
                ...
                style { r#"
                    body {
                        ...
                        background-image: url(""# (main_data.config.get_resource_root()) r#"/sb-title.png")
                    }
                    "#
                }
            }
