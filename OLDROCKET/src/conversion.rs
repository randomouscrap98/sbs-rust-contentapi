...
pub fn cast_result<T>(result: &api_data::RequestResult, name: &str) -> Result<Option<Vec<T>>, anyhow::Error> where T: for<'a> Deserialize<'a>  
{
...
pub fn cast_result_safe<T>(result: &api_data::RequestResult, name: &str) -> Result<Vec<T>, anyhow::Error> where T: for<'a> Deserialize<'a>  
{
    cast_result(result, name).and_then(|r| Ok(r.unwrap_or(Vec::new())))
}

//Cast result but throw error if the key isn't found
pub fn cast_result_required<T>(result: &api_data::RequestResult, name: &str) -> Result<Vec<T>, anyhow::Error> where T: for<'a> Deserialize<'a>  
{
    cast_result(result, name)?.ok_or(anyhow!("Couldn't find key {}", name)) //format!("Couldn't find key {}", name).into())
}
