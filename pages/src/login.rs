...
pub fn render(
    data: MainLayoutData,
    login_errors: Option<Vec<String>>,
    recover_errors: Option<Vec<String>>,
    email: Option<String>,
) -> String {
    layout(&data, html!{
        section {
...
impl Login {
    ...
    pub fn to_api_login(self, default_seconds: i32, long_seconds: i32) -> contentapi::forms::Login {
        ...
        contentapi::forms::Login {
            ...
            expireSeconds: if long_session {
                long_seconds.into()
            } else {
                default_seconds.into()
            },
        }
    }
}
...
pub async fn post_login_render(
    context: PageContext,
    login: &contentapi::forms::Login,
) -> (Response, Option<String>) {
    match context.api_context.post_login(login).await {
        Ok(token) => (
            Response::Redirect(context.layout_data.links.userhome()),
            Some(token),
        ),
        Err(error) => {
