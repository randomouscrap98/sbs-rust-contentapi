...
impl AdminRenderData {
    pub fn new_empty(data: MainLayoutData) -> Self {
        Self {
            data,
            frontpage: None,
            banner: None,
    ...
    pub fn new(
        data: MainLayoutData,
        frontpage: Option<Content>,
        banner: Option<Content>,
        docpage: Option<Content>,
        bans: Vec<UserBan>,
        logs: Vec<AdminLog>,
        users: HashMap<i64, User>,
    ) -> Self {
        let mut base = Self::new_empty(data);
...
async fn get_render_data(
    ...
    let result = context
        .api_context
        .post_request_profiled_opt(&request, "all_admin_logs")
        .await?;
...
pub async fn get_render(
    context: PageContext,
    search_params: AdminSearchParams,
) -> Result<Response, Error> {
    Ok(Response::Render(render(
        get_render_data(context, &search_params).await?,
        search_params,
    )))
}

async fn to_system_content(
    form: BasicPage,
    name: String,
    literal_type: String,
...
pub async fn post_frontpage(context: PageContext, form: BasicPage) -> Result<Response, Error> {
    ...
    let content = to_system_content(
        form,
        String::from("frontpage"),
        SBSPageType::FRONTPAGE.to_string(),
    )
    .await?;
...
pub async fn post_alert(mut context: PageContext, form: BasicPage) -> Result<Response, Error> {
    ...
    let content =
        to_system_content(form, String::from("alert"), SBSPageType::ALERT.to_string()).await?;

    match context.api_context.post_content(&content, None).await {
        Ok(new_alert) => {
            context.layout_data.raw_alert = new_alert.text;
        }
        Err(error) => errors.push(error.to_user_string()),
    };
    let mut render_data = get_base_render_data(context).await?;
...
pub async fn post_docscustom(context: PageContext, form: BasicPage) -> Result<Response, Error> {
    ...
    let content = to_system_content(
        form,
        String::from("docscustom"),
        SBSPageType::DOCSCUSTOM.to_string(),
    )
    .await?;
